...
namespace ConsoleApplication11
{
    class RelationGenerator
    {
        ...
        public static List<String> getRelatedEntities(String uri, int MaxNumberOfEntities=7)
        {
            ...
            sets.Add(endpoint.QueryWithResultSet("select ?z where {<" + uri + "> ?x ?z.?z <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://www.w3.org/2002/07/owl#Thing> } limit 1000"));
            sets.Add(endpoint.QueryWithResultSet("select ?z where {?z ?x <" + uri + ">.?z <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://www.w3.org/2002/07/owl#Thing> } limit 1000"));
            
            //first stage, get related with the same subject
            //Query to extract subjects
            SparqlResultSet cateogires = endpoint.QueryWithResultSet("select ?z where {<" + uri + "> <http://purl.org/dc/terms/subject> ?z} limit 1000");
            ...
            foreach (SparqlResult category in cateogires)
            {
                ...
                SparqlResultSet itemsInCategory = endpoint.QueryWithResultSet(
