...
namespace NLI
{
    ...
    class Lexicon
    {
        ...
        public List<LexiconPredicate> getPredicates(string question, int topN = 20, int Limit = 30)
        {
            ...
            List<String> newlyAddedWords = new List<string>();
            List<String> newlyAddedSynonyms = new List<string>();
            foreach (string item in permutationList)
            {
                newlyAddedWords.AddRange(ln.getStems(item));
            }
            permutationList.AddRange(newlyAddedWords);
            foreach (string item in permutationList)
            {
                newlyAddedSynonyms.AddRange(ln.getSynonyoms(item));
            }
            permutationList.AddRange(newlyAddedSynonyms);
            permutationList = permutationList.Distinct().ToList<string>();
            //END TESTING
