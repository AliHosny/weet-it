    ...
    class Lexicon
    {
        ...
        public List<LexiconPredicate> getPredicates(string question, int topN, int Limit)
        {
            ...
            foreach (string questionleft in permutationList)
            {
                ...
                remoteEndPoint.Timeout = 9999999;
                try
                {
        ...
        public List<LexiconLiteral> getLiterals(string question, int topN = 10, int Limit = 20)
        {
            ...
            foreach (string questionleft in permutationList)
            {
                ...
                // if redirections Query (Query2) returned something we skip the first Query 
                if (resultSet1.Count != 0 && resultSet2.Count != 0)
                {
                    resultSet = resultSet2.ToList<SparqlResult>().ToList(); 
                }
                else if (resultSet1.Count != 0 && resultSet2.Count == 0)
                {
                    resultSet = resultSet1.ToList<SparqlResult>().ToList(); 
                }
                ...
                foreach (SparqlResult result in resultSet)
                {
                    ...
                    string literalTypeOfOwner = (result.HasValue("typeOfOwner") && result.Value("typeOfOwner") != null  ) ? result.Value("typeOfOwner").ToString() : "";
                    ...
                    {
                        if (x.URI == resourceURI.ToString() && x.label == literalLabel.ToString() && x.QuestionMatch == questionleft)
                        {
                            exists = true;
                            if (x.typeOfOwner.Contains(literalTypeOfOwner))
                            {
                                totallyExists = true;
                                break;
                            }
                    ...
                    if (!exists)
                    {
                        LexiconLiteral tmpLexiconLiteral = new LexiconLiteral(resourceURI.ToString(), literalLabel.ToString(), questionleft, literalTypeOfOwner);
