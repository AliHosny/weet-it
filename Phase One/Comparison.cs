...
namespace Comparison_Part
{
    ...
    class Comparison
    {
        ...
        /// </summary>
        /// <param name="elementURI">List of comparison elements uris passed by the sparql query</param>
       ...
       /// </summary>
        ...
        string getLabel(INode node, string endpointLink)
        {
            if (node.NodeType.ToString() == "Uri")
            {
                ...
                SparqlResultSet results = endpoint.QueryWithResultSet("select ?x where {<" + node.ToString() + "> <http://www.w3.org/2000/01/rdf-schema#label> ?x FILTER (langMatches(lang(?x), \"EN\"))}");
                if (results.Count != 0)
