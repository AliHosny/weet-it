    ...
    class Comparison
    {
       ...
       /// </summary>
        ...
        void SetCommonPredicate()
        {
            ...
            for (int i = 0; i < comparisonElementUri.Count; i++)
            {
                predicateWithSubjectList.Add(QueryProcessor.ExecuteQueryWithString("select distinct ?x where{<" + comparisonElementUri[i] + "> ?x ?y}"));
                predicateWithObjectList.Add(QueryProcessor.ExecuteQueryWithString("select distinct ?x where{?y ?x <" + comparisonElementUri[i] + ">}"));
            }
